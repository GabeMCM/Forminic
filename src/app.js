import { APP_TOKENS } from './tokens/master.tokens.js';
import { initRhythmEditor } from './rhythm/editor.js';
import { elements } from './ui/elements.js';
const { tonicKeys, tonicLinkGrid, smartModeToggle, fieldControls, fieldName, fieldMinorButton, fieldFlatButton, fieldSharpButton, tonicModeDescription, tonicLinksDescription, degreeKeys, octaveValue, degreeCount, memoryCount, memoryGrid, soundState, audioStatus, soundSetSelect, rhythmPresetSelect, tempoControl, beatState, engineLabel, settingsDialog, helpDialog, mappingList, memoryDialog, memoryForm, memoryNameInput, memoryDialogTitle, memoryDialogDegrees, mappingSearch, shortcutCapture, captureActionName, captureShortcutPreview, confirmShortcut, cancelShortcut, themeToggle, themeLabel, composerWorkspace, performanceWorkspace, performanceMemoryGrid, baseMemoryGrid, stageCurrentNote, stageCurrentShape, stageRhythmPreset, stageTempoControl, autoRhythmButton, rhythmVariationButton, stageAutoRhythmButton, stageRhythmVariationButton, captureDialog, captureDialogNote, captureAsNote, captureAsBase, setList, setEmpty, newSetButton, updateSetButton, setDialog, setForm, setNameInput, setDialogSummary, previousSetButton, nextSetButton, setPosition, smartCommandModeInputs } = elements;

import { TONICS, NOTE_LETTERS, NATURAL_PITCHES, SCALE_PATTERNS, SCALE_DEGREES, DEGREES, SOUND_SETS, DEFAULT_BINDINGS } from './state/state.tokens.js';
import { state, loadBindings, loadMemories, loadPerformanceMemories, loadBaseMemories, loadSets, loadTonicLinks, loadSmartLinks } from './state/state.js';
...
function currentMidiNotes() {
  ...
  return [...new Set(notes)].slice(0, 9);
}
...
function pluckString(midi, index, total, startAt, velocity = 1, muted = false) {
  ...
  const humanVelocity = velocity * (0.94 + Math.random() * 0.1);
  const level = ((0.78 - index * 0.025) / Math.sqrt(Math.max(1, total * 0.85)))
    * humanVelocity;
  gain.gain.setValueAtTime(0.0001, startAt);
...
function dampVoices(release = 0.14) {
  ...
  state.voices.forEach(({ gain, source }) => {
...
function activate(action) {
  ...
  if (action.startsWith("smartTonic")) {
    ...
    if (state.degrees.size >= APP_TOKENS.MAX_DEGREES) {
